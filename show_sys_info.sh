    ...
    select input in cpu_load disk_load disk_use disk_inode mem_use tcp_status cpu_top10 mem_top10 traffic quit; do
        case $input in
            ...
            traffic)
                #查看网络流量
                while true; do
                    read -p "Please enter the network card name(ens[0-99] or em[0-99] or team[0-99]): " ens
		    ...
		    #CentOS7中RX行号是5，TX行号是7
                    ...
                    OLD_IN=`ifconfig $ens |awk -F'[: ]+' '/bytes/{if(NR==8)print $4;else if(NR==5)print $6}'`
                    OLD_OUT=`ifconfig $ens |awk -F'[: ]+' '/bytes/{if(NR==8)print $9;else if(NR==7)print $6}'`
                    sleep 1
                    NEW_IN=`ifconfig $ens |awk -F'[: ]+' '/bytes/{if(NR==8)print $4;else if(NR==5)print $6}'`
                    NEW_OUT=`ifconfig $ens |awk -F'[: ]+' '/bytes/{if(NR==8)print $9;else if(NR==7)print $6}'`
